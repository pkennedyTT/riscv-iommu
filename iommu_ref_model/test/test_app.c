...
main(void) {
    ...
    fail_if( (translate_gpa(DC.iohgatp, iommu.reg_file.fctl.gxl, DC.fsc.pdtp.PPN * PAGESIZE, &temp) == -1) );
    ...
    temp = translate_gpa(DC.iohgatp, iommu.reg_file.fctl.gxl, gpa, &temp);
    fail_if( (temp == -1) );
    ...
    gpte_addr = translate_gpa(DC.iohgatp, iommu.reg_file.fctl.gxl, (PC.fsc.iosatp.PPN * PAGESIZE), &temp);
    fail_if( (gpte_addr == -1) );
    read_memory_test(gpte_addr, 8, (char *)&gpte);
    gpte.V = 0;
    write_memory_test((char *)&gpte.raw, gpte_addr, 8);
